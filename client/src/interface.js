...
Interface.prototype.__enableListeners = function () {
  ...
  let createCharBtn = document.getElementById("create-character-confirm");
  if (createCharBtn) {
    createCharBtn.addEventListener("click", function () {
      let name = document.getElementById("create-char-name").value.toLowerCase();
      let sex = document.getElementById("create-char-sex").value;

      gameClient.networkManager.createCharacterExtra({
        name: name,
        sex: sex
      });
    }.bind(this));
...
Interface.prototype.showCharacterList = function (characters) {

  /*
   * Muestra el modal con el listado de personajes
   */

  let modal = this.modalManager.open("modal-character-list");
  let listContainer = modal.element.querySelector(".char-list");
  ...
  characters.forEach(ch => {
    ...
    row.addEventListener("click", function () {
      gameClient.networkManager.playCharacter(ch.name);
    });

    listContainer.appendChild(row);
  });

  // Botón "Create Character" abre el segundo modal
  let btnOpenCreate = modal.element.querySelector("#open-create-character");
  if (btnOpenCreate) {
    btnOpenCreate.onclick = function () {
      this.modalManager.open("modal-create-character");
    }.bind(this);
  }
};